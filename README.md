...
| `rust-performance` | Diagnose and improve Rust performance as a measured loop. Thin router (leverage order + smell table) over 10 topic references: profiling, benchmarking, build config, allocations, collections/hashing, data layout, iterators/bounds checks, parallelism, async, dispatch/inlining. | `rust-performance/SKILL.md` |
